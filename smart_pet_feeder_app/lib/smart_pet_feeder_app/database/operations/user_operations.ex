defmodule SmartPetFeederApp.UserOperations do
    ...
    def add(username, password, email, first_name, last_name) do
      user_changeset = %Users{} |> Users.changeset(%{username: username, password: password, email: email, first_name: first_name, last_name: last_name})
      
      case Repo.insert(user_changeset) do
        {:ok, user} ->
          {:ok, %{id: user.id, username: user.username, password: user.password, password_hash: user.password_hash, email: user.email, first_name: user.first_name, last_name: user.last_name}}
    ...
    def get() do
      case Repo.all(Users) do
        ...
        users ->
          u = for x <- users, do: %{id: x.id, username: x.username, email: x.email, first_name: x.first_name, last_name: x.last_name}
          {:ok, u}
      end
    end
    ...
    defp check_password(user, password) do
      case Bcrypt.checkpw(password, user.password_hash) do
        true -> {:ok, %{id: user.id, username: user.username, password: user.password, password_hash: user.password_hash, email: user.email, first_name: user.first_name, last_name: user.last_name}}
        false -> {:error, :incorrect_usr_or_pass}
      end
    end
